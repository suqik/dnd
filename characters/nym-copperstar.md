...
| 法术储存戒指 Spell-Storing Ring | 本次长休使用次数未消耗；已储存牧师的恢复法术（1 环；具体法术名称待补充） |
...
| 法术储存戒指 Spell-Storing Ring | 1 枚 | KEN 击败马特委托所称的黑暗势力首领、完成部分委托后，马特在旅馆会面时交付给 Nym；蛋蛋拒绝该戒指。这就是既有的同一枚戒指，并非第二枚。可储存 1 个 1 环法术，每次长休可使用 1 次；当前储存牧师的恢复法术，具体法术名称待补充 |
...
| 长休后（具体时段未记录） | 以 DM 奖励选择法术储存戒指的储存法术 | 当前储存牧师的恢复法术（1 环；具体法术名称待补充）。本次长休使用次数尚未消耗 |
